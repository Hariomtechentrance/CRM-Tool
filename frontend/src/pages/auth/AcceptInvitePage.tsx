...
const S = {
  ...
  card: { background: "#0D0D1F", border: "1px solid #1C1C35", borderRadius: 16, padding: "36px 40px", maxWidth: 440, width: "100%", textAlign: "center" as const, boxShadow: "0 24px 80px rgba(0,0,0,0.5)" },
      ...
      <div style={S.card}>
        ...
        {status === "loading" && (
          <>
            ...
            <p style={{ color: "#505070", fontSize: 14 }}>Loading invite details...</p>
          </>
        )}
        ...
        {status === "ready" && invite && (
          <>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#EEEEF5", margin: "0 0 8px" }}>You're Invited!</h2>
            <p style={{ fontSize: 14, color: "#505070", margin: "0 0 24px", lineHeight: 1.6 }}>
              <strong style={{ color: "#CCCCEE" }}>{invite.inviterName}</strong> invited you to join
            </p>
            <div style={{ background: "#131327", border: "1px solid #1C1C35", borderRadius: 12, padding: "16px 20px", marginBottom: 20 }}>
              <div style={{ fontSize: 18, fontWeight: 700, color: "#EEEEF5", marginBottom: 4 }}>{invite.orgName}</div>
            ...
            {!isAuthenticated && (
              <p style={{ fontSize: 12, color: "#505070", marginBottom: 4 }}>You'll need to log in or register first.</p>
            )}
            <button onClick={accept} style={S.btn}>
              {isAuthenticated ? "Accept & Join Organization" : "Log in to Accept Invite"}
            </button>
            <button onClick={() => navigate("/dashboard")} style={{ background: "none", border: "none", color: "#505070", fontSize: 12, cursor: "pointer", marginTop: 12, width: "100%" }}>
              Decline
            </button>
          </>
        )}
        ...
        {status === "accepting" && (
          <>
            ...
            <p style={{ color: "#505070", fontSize: 14 }}>Joining organization...</p>
          </>
        )}
        ...
        {status === "success" && (
          <>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#EEEEF5", margin: "0 0 8px" }}>Welcome aboard!</h2>
            <p style={{ fontSize: 14, color: "#505070" }}>You've successfully joined <strong style={{ color: "#CCCCEE" }}>{invite?.orgName}</strong>. Redirecting...</p>
          </>
        )}
        ...
        {status === "error" && (
          <>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#EEEEF5", margin: "0 0 8px" }}>Invite Invalid</h2>
            <p style={{ fontSize: 14, color: "#505070", marginBottom: 20 }}>{errorMsg}</p>
