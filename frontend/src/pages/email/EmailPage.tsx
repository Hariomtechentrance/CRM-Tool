...
const S = {
  ...
  kpiValue: { fontSize: 28, fontWeight: 700, color: "#EEEEF5", margin: "4px 0 0" } as React.CSSProperties,
          ...
          {loading ? <div style={{ padding: 40, textAlign: "center", color: "#505070" }}>Loading...</div> : (
            <div className="table-wrap">
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {filtered.length === 0
                    ? <tr><td colSpan={7} style={{ ...S.td, textAlign: "center", color: "#505070", padding: 40 }}>No emails yet. Compose your first email!</td></tr>
                    : filtered.map(e => (
                      <tr key={e.id}>
                        <td style={{ ...S.td, color: "#EEEEF5" }}>{e.toEmail}{e.ccEmail && <span style={{ color: "#505070", fontSize: 11 }}> +cc</span>}</td>
                        ...
                        <td style={S.td}>
                          <div style={{ display: "flex", gap: 6 }}>
                            <button onClick={() => setPreviewEmail(e)} style={{ background: "none", border: "none", color: "#505070", cursor: "pointer", padding: 4 }} title="Preview"><Eye size={14} /></button>
                            <button onClick={() => deleteEmail(e.id)} style={{ background: "none", border: "none", color: "#505070", cursor: "pointer", padding: 4 }} title="Delete"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      {tab === "templates" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
          {templates.length === 0 && !loading && (
            <div style={{ gridColumn: "1/-1", ...S.card, textAlign: "center", padding: 48, color: "#505070" }}>
              <FileText size={36} style={{ margin: "0 auto 12px", opacity: 0.4 }} />
              <p style={{ fontSize: 14, fontWeight: 600, color: "#CCCCEE" }}>No templates yet</p>
          ...
          {templates.map(t => (
            <div key={t.id} style={{ ...S.card, cursor: "default" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <div>
                  <p style={{ fontSize: 14, fontWeight: 700, color: "#EEEEF5", margin: "0 0 4px" }}>{t.name}</p>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  ...
                  <button onClick={() => openEditTemplate(t)} style={{ background: "#131327", border: "1px solid #1C1C35", color: "#CCCCEE", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11 }}>Edit</button>
                  <button onClick={() => deleteTemplate(t.id)} style={{ background: "none", border: "none", color: "#505070", cursor: "pointer", padding: 4 }}><Trash2 size={13} /></button>
                </div>
              </div>
              <p style={{ fontSize: 12, color: "#818CF8", fontWeight: 600, marginBottom: 6 }}>Subject: {t.subject}</p>
              <p style={{ fontSize: 12, color: "#505070", lineHeight: 1.6, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical" as any }}>{t.body}</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }}
          ...
          <div className="modal-inner" style={{ maxWidth: 560 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ color: "#EEEEF5", margin: 0, fontSize: 16, fontWeight: 700, display: "flex", alignItems: "center", gap: 8 }}><Mail size={18} color="#818CF8" /> Compose Email</h3>
              <button onClick={() => setShowCompose(false)} style={{ background: "none", border: "none", color: "#505070", cursor: "pointer" }}><X size={18} /></button>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "flex-end" }}>
              <button onClick={() => setShowCompose(false)} style={{ ...S.btn, background: "#1C1C35", color: "#CCCCEE" }}>Cancel</button>
      ...
      {showTemplate && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }}
          ...
          <div className="modal-inner" style={{ maxWidth: 520 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ color: "#EEEEF5", margin: 0, fontSize: 16, fontWeight: 700 }}>{editTemplateId ? "Edit Template" : "New Template"}</h3>
              <button onClick={() => setShowTemplate(false)} style={{ background: "none", border: "none", color: "#505070", cursor: "pointer" }}><X size={18} /></button>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "flex-end" }}>
              <button onClick={() => setShowTemplate(false)} style={{ ...S.btn, background: "#1C1C35", color: "#CCCCEE" }}>Cancel</button>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }}
          ...
          <div className="modal-inner" style={{ maxWidth: 560 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h3 style={{ color: "#EEEEF5", margin: 0, fontSize: 15, fontWeight: 700 }}>Email Preview</h3>
              <button onClick={() => setPreviewEmail(null)} style={{ background: "none", border: "none", color: "#505070", cursor: "pointer" }}><X size={18} /></button>
            </div>
            <div style={{ background: "#131327", borderRadius: 8, padding: 16, marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: "#505070", marginBottom: 6 }}>
                <strong style={{ color: "#CCCCEE" }}>To:</strong> {previewEmail.toEmail}
                {previewEmail.ccEmail && <> &nbsp;|&nbsp; <strong style={{ color: "#CCCCEE" }}>CC:</strong> {previewEmail.ccEmail}</>}
              </div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#EEEEF5", marginBottom: 12 }}>{previewEmail.subject}</div>
              <div style={{ fontSize: 13, color: "#CCCCEE", lineHeight: 1.7, whiteSpace: "pre-wrap", borderTop: "1px solid #1E1E38", paddingTop: 12 }}>{previewEmail.body}</div>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 12, color: "#505070" }}>{previewEmail.sentAt ? `Sent: ${new Date(previewEmail.sentAt).toLocaleString("en-IN")}` : `Created: ${new Date(previewEmail.createdAt).toLocaleString("en-IN")}`}</span>
