    ...
    <div style={S.page}>
      ...
      <div style={S.card}>
        <div style={S.toolbar}>
          ...
          <span style={{ fontSize: 12, color: "var(--text-ghost)" }}>{total} organizations</span>
        </div>
        ...
        {loading ? <div style={{ padding: 40, textAlign: "center", color: "var(--text-ghost)" }}>Loading...</div> : (
          <table style={S.table}>
            ...
            <tbody>
              {orgs.length === 0 ? (
                <tr><td colSpan={8} style={{ ...S.td, textAlign: "center", color: "var(--text-ghost)", padding: 32 }}>No organizations found.</td></tr>
              ) : orgs.map(o => (
                <tr key={o.id}>
                  <td style={S.td}>
                    <div style={{ fontWeight: 600, color: "var(--text-primary)" }}>{o.name}</div>
                    <div style={{ fontSize: 11, color: "var(--text-ghost)" }}>{o.slug}</div>
        ...
        <div style={S.modal} onClick={e => e.target === e.currentTarget && setSelected(null)}>
          <div style={S.modalBox}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ color: "var(--text-primary)", margin: 0, fontSize: 16, fontWeight: 700 }}>Edit: {selected.name}</h3>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ background: "var(--bg-hover)", borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 11, color: "var(--text-ghost)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>Organization Stats</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {[["Members", selected._count.members], ["Parties", selected._count.parties], ["Invoices", selected._count.invoices]].map(([k, v]) => (
                    <div key={String(k)} style={{ textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 10, color: "var(--text-ghost)" }}>{k}</div>
                    </div>
                  ))}
                </div>
              </div>
