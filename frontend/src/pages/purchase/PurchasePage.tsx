                  ...
                  {items.map((it, i) => (
                    <div key={i} style={{ background: "#131327", borderRadius: 8, padding: 12 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr auto", gap: 8, alignItems: "end" }}>
                        ...
                        <div><label style={{ ...S.label, marginBottom: 4 }}>Qty</label><input type="number" style={S.input} value={it.quantity} onChange={(e) => setItem(i, "quantity", e.target.value)} /></div>
                        <div><label style={{ ...S.label, marginBottom: 4 }}>Rate (₹)</label><input type="number" style={S.input} value={it.unitPrice} onChange={(e) => setItem(i, "unitPrice", e.target.value)} /></div>
