...
final _purchaseOrderProvider = FutureProvider.family<Map<String, dynamic>, String>((ref, id) async {
  final res = await ApiClient().getPurchaseOrderDetail(id);
...
class _PurchaseOrderDetailScreenState extends ConsumerState<PurchaseOrderDetailScreen> {
  ...
  Future<void> _updateStatus(Map<String, dynamic> order, String newStatus) async {
    try {
      await ApiClient().updatePurchaseOrder(order['id'] as String, {'status': newStatus});
      ref.invalidate(_purchaseOrderProvider(widget.id));
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    final order = ref.watch(_purchaseOrderProvider(widget.id));

    return Scaffold(
      backgroundColor: AppColors.bgLight,
      appBar: AppBar(
        title: order.maybeWhen(
          data: (d) => Text(d['poNumber'] as String? ?? d['orderNumber'] as String? ?? 'Purchase Order'),
  ...
  Widget _buildBody(Map<String, dynamic> d) {
    ...
    final vendor = d['vendor']?['name'] as String? ?? d['supplierName'] as String? ?? 'N/A';
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(d['poNumber'] as String? ?? d['orderNumber'] as String? ?? '—',
                style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white)),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                decoration: BoxDecoration(color: sc.withOpacity(0.9), borderRadius: BorderRadius.circular(8)),
                child: Text(status, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800)),
              ),
            ]),
            const SizedBox(height: 10),
            Row(children: [
              const Icon(Icons.store_outlined, size: 14, color: Colors.white70),
              const SizedBox(width: 6),
              Text(vendor, style: const TextStyle(fontSize: 13, color: Colors.white)),
            ]),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Wrap(spacing: 8, runSpacing: 8,
              children: ['SENT', 'PARTIAL', 'RECEIVED', 'CANCELLED'].map((s) {
        ...
        _infoCard('Order Information', [
          _infoRow('Vendor',       vendor),
          if (d['expectedDate'] != null) _infoRow('Expected Date', d['expectedDate'] as String),
        ...
        if (items.isNotEmpty) ...[
          ...
          ...items.map((item) {
            ...
            final recv = it['receivedQty'] as num?;
            return Container(
              ...
              child: Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text('Ordered: $qty${recv != null ? '  •  Received: $recv' : ''}  •  ₹${_fmt.format(rate)}/unit',
